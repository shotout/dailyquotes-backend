    ...
    if (validateEmail(email)) {
      ...
      $.ajax({
        ...
        success: function(data) {
          if (data != "true") {
            $("#val_email").css('display', 'block').text('This email is already existed.');
            $('#submitBtn').on("click", function(e) {
              e.preventDefault();
            });
            result = 0;
  ...
  $(document).ready(function() {
    ...
    $("#validatedCustomFile").on("change", function() {
      ...
      if ($.inArray(ext, ['csv']) == -1) {
        ...
        $('#note_txt_2').show();
        $('#note_txt_2').html('<h6> <span class="text-danger font-weight-bolder"> '+ 'Invalid file Extension.' +' </span> </h6> <span class="badge badge-info note-style">'+ 'Note' +'</span><small class="text-info">'+ ' Allowed File Extensions: csv') + '</small>';
    ...
    $(document).ready(function() {
      $('#confirmDelete').on('show.bs.modal', function(e) {
        ...
        if (button.data("label") == 'Delete') {
          modal.find('#confirmDeleteSubmitBtn').addClass('delete-task-btn').attr('data-task', button.data('id')).text('Delete').show();
...
if (($('.main-body .page-wrapper').find('#customer-task-container').length)) {
  ...
  $(document).ready(function() {
    $('#confirmDelete').on('show.bs.modal', function(e) {
      ...
      $('#confirmDeleteSubmitBtn').attr('data-task', '').removeClass('delete-task-btn');
      if (button.data("label") == 'Delete') {
        modal.find('#confirmDeleteSubmitBtn').addClass('btn btn-danger custom-btn-small delete-task-btn').attr('data-task', button.data('id')).text('Delete').show();
        modal.find('#confirmDeleteLabel').text(button.data('title'));
        modal.find('.modal-body').text(button.data('message'));
      }
      $('#confirmDeleteSubmitBtn').on('click', function() {
...
if (($('.main-body .page-wrapper').find('#customer-ticket-container').length)) {
  ...
  $(document).ready(function() {
    $('#confirmDelete').on('show.bs.modal', function(e) {
      ...
      $('#confirmDeleteSubmitBtn').attr('data-task', '').removeClass('delete-task-btn');
      if (button.data("label") == 'Delete') {
        modal.find('#confirmDeleteSubmitBtn').addClass('btn-danger custom-btn-small delete-task-btn').attr('data-task', button.data('id')).text('Delete').show();
        modal.find('#confirmDeleteLabel').text(button.data('title'));
        modal.find('.modal-body').text(button.data('message'));
      }
      $('#confirmDeleteSubmitBtn').on('click', function() {
...
if (($('.main-body .page-wrapper').find('.list-container').length)) {
  $(document).ready(function() {
    $('#dataTableBuilder').on('change', '.status', function() {
      ...
      var url = SITE_URL + "/admin/change-status2";
      $.ajax({
        url: url,
        method: "POST",
        data: {
...
if ($('.main-body .page-wrapper').find('#area-list-quote').length) {
