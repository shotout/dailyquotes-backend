...
function toggleMultiVariants() {
	if ($("#multi_variants").is(":checked")) {
		...
		$("#rdoAttribute").html('Attributes');
	} else {
		$("#multiVariantsBlock").hide();
		$("#rdoAttribute").html('Enable Attribute');
	}
}
...
function toggleManageStock() {
	if ($('#manageStock').is(":checked")) {
		...
		$("#rdoStock").html('Current Stocks');
	} else {
		$('#initialStockBlock').hide();
		$("#rdoStock").html('Manage Stock Level');
	}
}
			...
			$.ajax({
				...
				.done(function(data) {
					...
					if (data.status_no == 1) {
						...
						$("#checkMsg").html('This name is available to use');
	...
	function customVariantTitleValidation() {
        ...
        $('.customVariantDiv').each(function() {
			...
			if (text_input_check == 0) {
				...
				$('#extra-variant-value-' + i).text('This field is required.');
				fl = 1;
			} else {
			...
			if (txt_value_title == 0) {
				...
				$('#extra-variant-title-' + i).text('This field is required.');
				temp = 1;
			} else {
...
if (($('.main-body .page-wrapper').find('#edit-item-container').length)) {
	...
	$('#confirmDelete').on('show.bs.modal', function(e) {
		...
		if (button.data("label") == 'Delete') {
			modal.find('#confirmDeleteSubmitBtn').addClass('delete-task-btn').attr('data-id', button.data('id')).text('Delete').show();
		...
		$.ajax({
			...
			success: function(data) {
				...
				if (msg == '1') {
					...
					$('#msgStatus').fadeIn();
					$('#msgStatus').html('Stock Management Disabled').css('color', 'red');
				}
			}
		});
	});
...
if (($('.main-body .page-wrapper').find('#import-item-container').length)) {
	...
	$("#validatedCustomFile").on("change", function() {
		...
		if ($.inArray(ext, ['csv']) == -1) {
			$('#note_txt_1').hide();
			$('#note_txt_2').show();
			$('#note_txt_2').html('<h6> <span class="text-danger font-weight-bolder"> ' + 'Invalid file Extension.' + ' </span> </h6> <span class="badge badge-danger">' + 'Note' + '!</span>' + 'Allowed File Extensions: csv');
...
if (($('.main-body .page-wrapper').find('.list-container').length)) {
	...
	$('#dataTableBuilder').on('change', '.status2', function() {
		var id = $(this).attr('data-id');
	...
	$('#dataTableBuilder').on('change', '.status3', function() {
		var id = $(this).attr('data-id');
